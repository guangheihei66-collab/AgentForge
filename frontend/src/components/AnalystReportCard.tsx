...
export function AnalystReportCard({ analyst }: Props) {
  ...
  return <section className="panel analyst-panel" aria-label={heading}>
    ...
    <div className="analyst-coverage"><CheckCircle2 size={15} /><span>{t('analyst.evidenceCoverage', { referenced: report.evidence_coverage.referenced_count, available: report.evidence_coverage.available_count })}</span>{report.evidence_coverage.truncated && <span className="analyst-truncated">{t('analyst.truncated')}</span>}</div>
  </section>
}
