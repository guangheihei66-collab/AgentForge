...
export function App() {
  ...
  return <div translate="no"><Shell page={page} setPage={setPage} pending={ops.approvals.length} languageSelector={<LanguageSelector />}>
    ...
    {page === 'dashboard' && <Dashboard tasks={ops.tasks} approvals={ops.approvals} providerStatus={ops.providerStatus} testingProvider={ops.testingProvider} onTestProvider={() => void ops.testProviderConnection()} onTask={async (id) => { await ops.chooseTask(id); setPage('detail') }} onApprovals={() => setPage('approvals')} />}
    {page === 'projects' && <Projects projects={ops.projects} onOpen={async (id) => { await ops.chooseProject(id); setPage('project-detail') }} onCreate={ops.createProject} onValidate={ops.validateWorkspace} />}
