...
export function App() {
  ...
  return <Shell page={page} setPage={setPage} pending={ops.approvals.length}>
    ...
    {page === 'agent' && <AgentWorkspace projects={ops.projects} planning={ops.agentPlanning} error={ops.agentError} onStart={async (projectId, goal) => { await ops.createAgentTask(projectId, goal) }} approvals={ops.approvals} onApprove={async (id) => { await ops.act('approve', id) }} onReject={async (id) => { await ops.act('reject', id) }} />}
