...
Dim fso, shell, root, commandName, pythonw, entry, command
Set fso = CreateObject("Scripting.FileSystemObject")
Set shell = CreateObject("WScript.Shell")
root = fso.GetParentFolderName(fso.GetParentFolderName(WScript.ScriptFullName))
commandName = "exit"
If WScript.Arguments.Count > 0 Then commandName = LCase(WScript.Arguments(0))
If commandName <> "show_or_open" And commandName <> "stop_services" And commandName <> "restart_services" And commandName <> "exit" Then
  WScript.Quit 2
End If
pythonw = ResolvePythonW(root)
If pythonw = "" Then WScript.Quit 1
entry = fso.BuildPath(root, "launcher\command.pyw")
If Not fso.FileExists(entry) Then WScript.Quit 1
command = Quote(pythonw) & " " & Quote(entry) & " --root " & Quote(root) & " --command " & Quote(commandName)
